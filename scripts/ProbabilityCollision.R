...
#### Now calculate downwind length
	...
	(sampledBirdParams$WingSpan[i] * Flap_Glide * CollisionRiskTab$alpha[u + 1]) + 	
		abs(-sampledTurbine$BladeWidth[i]*CollisionRiskTab$chord[i + 1]*sin(Pitch[1])+
			(CollisionRiskTab$alpha[u + 1] * sampledTurbine$BladeWidth[i]*CollisionRiskTab$chord[i + 1]*cos(Pitch[1])))
